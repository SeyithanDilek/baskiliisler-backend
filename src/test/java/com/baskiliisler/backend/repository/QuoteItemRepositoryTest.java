package com.baskiliisler.backend.repository;

import com.baskiliisler.backend.model.*;
import com.baskiliisler.backend.type.QuoteStatus;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.autoconfigure.orm.jpa.DataJpaTest;
import org.springframework.boot.test.autoconfigure.orm.jpa.TestEntityManager;

import java.math.BigDecimal;
import java.time.LocalDate;
import java.time.LocalDateTime;
import java.util.ArrayList;
import java.util.List;
import java.util.Optional;

import static org.assertj.core.api.Assertions.assertThat;

@DataJpaTest
@DisplayName("QuoteItemRepository Test")
class QuoteItemRepositoryTest {

    @Autowired
    private TestEntityManager entityManager;

    @Autowired
    private QuoteItemRepository quoteItemRepository;

    private QuoteItem testQuoteItem;
    private Quote testQuote;
    private Product testProduct;

    @BeforeEach
    void setUp() {
        Brand testBrand = Brand.builder()
                .name("Test Brand")
                .contactEmail("test@brand.com")
                .contactPhone("1234567890")
                .build();
        
        entityManager.persist(testBrand);

        testQuote = Quote.builder()
                .brand(testBrand)
                .status(QuoteStatus.OFFER_SENT)
                .validUntil(LocalDate.now().plusDays(30))
                .createdAt(LocalDateTime.now())
                .currency("TRY")
                .totalPrice(BigDecimal.valueOf(1000))
                .items(new ArrayList<>())
                .build();
        
        entityManager.persist(testQuote);

        testProduct = Product.builder()
                .name("Test Product")
                .code("TEST_PROD")
                .unit("adet")
                .unitPrice(BigDecimal.valueOf(100))
                .active(true)
                .build();
        
        entityManager.persist(testProduct);

        testQuoteItem = QuoteItem.builder()
                .quote(testQuote)
                .product(testProduct)
                .quantity(10)
                .unitPrice(BigDecimal.valueOf(100))
                .lineTotal(BigDecimal.valueOf(1000))
                .build();

        entityManager.persistAndFlush(testQuoteItem);
    }

    @Test
    @DisplayName("Teklif kalemi ID'ye göre bulma")
    void whenFindById_thenReturnQuoteItem() {
        // when
        Optional<QuoteItem> result = quoteItemRepository.findById(testQuoteItem.getId());

        // then
        assertThat(result).isPresent();
        assertThat(result.get().getQuantity()).isEqualTo(10);
        assertThat(result.get().getUnitPrice()).isEqualTo(BigDecimal.valueOf(100));
        assertThat(result.get().getLineTotal()).isEqualTo(BigDecimal.valueOf(1000));
        assertThat(result.get().getProduct().getName()).isEqualTo("Test Product");
        assertThat(result.get().getQuote().getBrand().getName()).isEqualTo("Test Brand");
    }

    @Test
    @DisplayName("Olmayan teklif kalemi ID'ye göre arama")
    void whenFindById_withNonExistingId_thenReturnEmpty() {
        // when
        Optional<QuoteItem> result = quoteItemRepository.findById(999L);

        // then
        assertThat(result).isEmpty();
    }

    @Test
    @DisplayName("Tüm teklif kalemlerini listeleme")
    void whenFindAll_thenReturnAllQuoteItems() {
        // given
        Product product2 = Product.builder()
                .name("Product 2")
                .code("PROD_2")
                .unit("kg")
                .unitPrice(BigDecimal.valueOf(200))
                .active(true)
                .build();
        
        entityManager.persist(product2);

        QuoteItem quoteItem2 = QuoteItem.builder()
                .quote(testQuote)
                .product(product2)
                .quantity(5)
                .unitPrice(BigDecimal.valueOf(200))
                .lineTotal(BigDecimal.valueOf(1000))
                .build();

        entityManager.persistAndFlush(quoteItem2);

        // when
        List<QuoteItem> result = quoteItemRepository.findAll();

        // then
        assertThat(result).hasSize(2);
        assertThat(result).extracting(QuoteItem::getQuantity)
                .containsExactlyInAnyOrder(10, 5);
        assertThat(result).extracting(item -> item.getProduct().getName())
                .containsExactlyInAnyOrder("Test Product", "Product 2");
    }

    @Test
    @DisplayName("Teklif kalemi kaydetme")
    void whenSave_thenReturnSavedQuoteItem() {
        // given
        Product newProduct = Product.builder()
                .name("New Product")
                .code("NEW_PROD")
                .unit("lt")
                .unitPrice(BigDecimal.valueOf(50))
                .active(true)
                .build();
        
        entityManager.persist(newProduct);

        QuoteItem newQuoteItem = QuoteItem.builder()
                .quote(testQuote)
                .product(newProduct)
                .quantity(20)
                .unitPrice(BigDecimal.valueOf(50))
                .lineTotal(BigDecimal.valueOf(1000))
                .build();

        // when
        QuoteItem result = quoteItemRepository.save(newQuoteItem);

        // then
        assertThat(result).isNotNull();
        assertThat(result.getId()).isNotNull();
        assertThat(result.getQuantity()).isEqualTo(20);
        assertThat(result.getUnitPrice()).isEqualTo(BigDecimal.valueOf(50));
        assertThat(result.getLineTotal()).isEqualTo(BigDecimal.valueOf(1000));
        assertThat(result.getProduct().getName()).isEqualTo("New Product");
        assertThat(result.getQuote()).isEqualTo(testQuote);
    }

    @Test
    @DisplayName("Teklif kalemi silme")
    void whenDelete_thenQuoteItemDeleted() {
        // given
        QuoteItem saved = entityManager.persistAndFlush(QuoteItem.builder()
                .quote(testQuote)
                .product(testProduct)
                .quantity(15)
                .unitPrice(BigDecimal.valueOf(75))
                .lineTotal(BigDecimal.valueOf(1125))
                .build());

        // when
        quoteItemRepository.deleteById(saved.getId());
        entityManager.flush();

        // then
        Optional<QuoteItem> result = quoteItemRepository.findById(saved.getId());
        assertThat(result).isEmpty();
    }
} 